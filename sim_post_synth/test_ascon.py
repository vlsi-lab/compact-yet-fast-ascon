...
def read_lengths(filename):
    with open(filename, "r") as f:
        aad_len = int(f.readline().strip())
        msg_len = int(f.readline().strip())
    return aad_len, msg_len
...
# Lettura del plaintext (già fatto prima)
    ...
    aad = read_hex_file("aad_data.hex")
    plaintext = read_hex_file("msg_data.hex")

    aad_len, msg_len = read_lengths("lengths.txt")

    # Sanity check
    assert len(aad) == aad_len, f"AAD length mismatch: expected {aad_len}, got {len(aad)}"
    assert len(plaintext) == msg_len, f"MSG length mismatch: expected {msg_len}, got {len(plaintext)}"
